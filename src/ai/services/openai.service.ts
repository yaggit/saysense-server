...
import {
  AnalysisMetric,
  AnalysisMetricType,
} from '../../common/entities/analysis-metric.entity';
import {
  SuggestionType,
  SuggestionSeverity,
  FeedbackSuggestion,
} from '../../common/entities/feedback-suggestion.entity';
...
export class OpenAIService implements IAiService {
  ...
  async transcribeAudio(
    ...
    try {
      ...
      this.logger.log(
        `Transcribing audio (${audioData.length} bytes) in ${language}`,
      );
  ...
  async analyzeSentiment(
    ...
    try {
      this.logger.log(
        `Analyzing sentiment for text (${text.length} chars) in ${language}`,
      );
  ...
  async analyzeAudio(
    ...
    try {
      ...
      const session = await this.sessionsService.getSessionForUser(
        sessionId,
        'system',
      );
      ...
      const metrics: Omit<AnalysisMetric, 'id' | 'createdAt' | 'sessionId'>[] =
        [
          {
            metricType: AnalysisMetricType.TONE,
          ...
          {
            metricType: AnalysisMetricType.SPEED,
          ...
          {
            metricType: AnalysisMetricType.CLARITY,
  ...
  async generateFeedback(
    ...
    const speedMetric = metrics.find(
      (m) => m.metricType === AnalysisMetricType.SPEED,
    );
    if (speedMetric) {
      if (speedMetric.value > 180) {
        const suggestion = new FeedbackSuggestion({
          ...
          message:
            "You're speaking very quickly. Try to slow down for better clarity.",
        ...
        const suggestion = new FeedbackSuggestion({
          type: SuggestionType.PACING,
          message:
            "You're speaking quite slowly. Try to maintain a moderate pace to keep your audience engaged.",
